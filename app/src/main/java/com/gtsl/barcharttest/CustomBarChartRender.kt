...
//            }
    ...
    override fun drawValue(
        ...
        if (text?.contains(".") == true) {
            splitText = text.split(",".toRegex()).toTypedArray()
            val paintStyleOne = Paint(mValuePaint)
            val paintStyleTwo = Paint(mValuePaint)
            paintStyleOne.setColor(Color.RED)
            paintStyleTwo.setColor(Color.BLUE)

            val background = getTextBackgroundSize(x, y-20f, text, paintStyleOne)
            c?.drawRect(background, paintStyleTwo)
    ...
    private fun getTextBackgroundSize(
        ...
        val halfTextLength = paint.measureText(text) / 2 + 10
        return Rect(
            (x - halfTextLength).toInt(),
            (y + fontMetrics.top - 5).toInt(),
            (x + halfTextLength).toInt(),
            (y + fontMetrics.bottom + 5).toInt()
        )
    }
